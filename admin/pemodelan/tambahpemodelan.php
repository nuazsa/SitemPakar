  ...
  <main id="main" class="main">
    ...
    <div class="row">
      ...
      if (isset($_POST['simpan'])) {
        ...
        $query = mysqli_query($koneksi, "SELECT max(kode_model) as kodeTerbesar FROM model");
        ...
        //cek keberadaan data
        $sqlrs = mysqli_query($koneksi, "SELECT kode_model FROM model WHERE kode_model='$kode'");
        $rs = mysqli_num_rows($sqlrs);
        if ($rs == 0) {
          $perintah = "INSERT INTO model(kode_model,nama_model,solusi)VALUES('$kode','$nama_penyakit','$solusi')";
          $berhasil = mysqli_query($koneksi, $perintah);

          //jika data berhasil disimpan
          if ($berhasil) {
            echo '<script>alert("Data berhasil disimpan"); window.location.href="./pemodelan.php";</script>'   ;
          } else {
