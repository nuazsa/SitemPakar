...
if ($kdubah != "") {
  #menampilkan data
  $sql = "SELECT * FROM gangguan WHERE kode_gangguan='$kdubah'";
  $qry = mysqli_query($koneksi, $sql)
    or die("SQL ERROR" . mysqli_error($koneksi));
  $data = mysqli_fetch_array($qry);
  #samakan dengan variabel form
  $id = $data['kode_gangguan'];
  $edit_penyakit = $data['nama_gangguan'];
...
<body>
  ...
  <?php 
    if (isset($_POST['simpan'])) {
      ...
      $sql = "UPDATE gangguan SET nama_gangguan='$penyakit', solusi='$solusi' WHERE kode_gangguan='$id'";
      $result = mysqli_query($koneksi, $sql) or die("SQL Error" . mysqli_error($koneksi));

      if ($result) {
        echo '<script>alert("Data berhasil diubah"); window.location.href="./penyakit.php";</script>'   ;
      } else {
